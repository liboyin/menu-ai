...
3. For each item, [`searchDishImage`](src/lib/menu-processor.ts) calls the RapidAPI Real-time Image Search endpoint by dish name and attaches the first result URL. Failures (missing key, network error, no result) resolve to `https://placehold.co/600x400.png?text=Image+Not+Found` rather than throwing.
...
- **SVG dish images are blocked.** `next.config.js` leaves `dangerouslyAllowSVG` off (default), so SVG URLs returned by RapidAPI render as broken images rather than being proxied. This closes an XSS vector since SVG can contain `<script>` and `remotePatterns` is a wildcard. The placeholder URL forces `.png` for the same reason — `placehold.co` defaults to SVG without an explicit extension.
